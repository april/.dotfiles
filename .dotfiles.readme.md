...
for repo in zsh-users/zsh-completions zsh-users/zsh-autosuggestions zsh-users/zsh-syntax-highlighting; do
  name="${repo#*/}"
  if [ ! -d "$ZSH_CUSTOM/plugins/$name" ]; then
    git clone --depth=1 "https://github.com/$repo.git" \
      "$ZSH_CUSTOM/plugins/$name" || echo "$name clone failed (non-fatal)"
  fi
done
```
